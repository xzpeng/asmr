...
class ProductionAction extends CommonAction
{
    public function index()
    {
        ...
        $field=array('id','category','subcategory','title','img','discount','display','href','top','original_price','discount_price','cname','cqq','cmobile','cemail');
        $result=$m->where($map)->order($order)->field($field)->limit($paging->start,$paging->displayrows)->select();
        for($i=0;$i<count($result);$i++)
        {
            $result[$i]['img']=__ROOT__.'/Public/Uploads/Home/Production/'.$result[$i]['img'];
            $result[$i]['haveHref']=$result[$i]['href']?'#f00':'#666';
            //$result[$i]['href']=$result[$i]['href']?'href="'.$result[$i]['href'].'"':"";
            //$result[$i]['typeInfo'] = explode(':', $result['type']);
        }
