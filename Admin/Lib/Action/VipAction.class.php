...
class VipAction extends CommonAction
{
    public function index()
    {
        ...
        $field=array('id','name','idcard_a','idcard_b','auth','name','qq','mobile','register_time');
        $result=$m->where($map)->order($order)->field($field)->limit($paging->start,$paging->displayrows)->select();
        for($i=0;$i<count($result);$i++)
        {
            $result[$i]['idcard_a']=__ROOT__.'/Public/Uploads/Home/Idcard/'.$result[$i]['idcard_a'];
            $result[$i]['idcard_b']=__ROOT__.'/Public/Uploads/Home/Idcard/'.$result[$i]['idcard_b'];
        }
